...
- (void)setIsAddNewGoodsCell:(BOOL)isAddNewGoodsCell {
    ...
    if (isAddNewGoodsCell) {
        ...
        [self.goodsTipsLab setHidden:NO];
    }else {
        [self.deleteBtn setHidden:NO];
        [self.goodsTipsLab setHidden:YES];
...
- (void)chooseImageAction:(UITapGestureRecognizer *)gesture {
    if (gesture.numberOfTouches == 1) {
        if ([self.imageViewCellDelegate conformsToProtocol:@protocol(WZ_ConfigGoodsImageViewCellDelegate)]
            &&
            [self.imageViewCellDelegate respondsToSelector:@selector(WZ_ConfigGoodsImageViewCell_ChoosePhotoWithIndexPath:andIsAddNewImage:)]) {
            [self.imageViewCellDelegate WZ_ConfigGoodsImageViewCell_ChoosePhotoWithIndexPath:self.indexPath andIsAddNewImage:self.isAddNewGoodsCell];
        }
    }
}
