...
@interface YuyueDetailEditVC ()<UITableViewDelegate,UITableViewDataSource,ShopEditCellDeledate>
@property(nonatomic,strong)UITableView* tableView;
@end

@implementation YuyueDetailEditVC
...
-(void)setModel:(PartModel *)model{
    ...
    if (self.datasArry.count==0) {
        NSMutableArray *codeArr=[NSMutableArray arrayWithObjects:@"yysj1",@"yysj2",@"yysj3",@"yysj4", nil];
        NSMutableArray *valueArr=[NSMutableArray arrayWithObjects:@"联系人姓名",@"联系人手机号码",@"需要试驾的车型",@"试驾的时间",nil];
        NSMutableArray *typeArr=[NSMutableArray arrayWithObjects:@"1",@"1",@"1",@"2", nil];
        for (int i=0; i<4; i++) {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    return self.datasArry.count-4;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    
    if (indexPath.section==0) {
        NSString *cellId = @"shijiaCell";
        YuyueShiJiaCell *cell = [tableView dequeueReusableCellWithIdentifier:cellId];
        if (!cell) {
            cell = [[[NSBundle mainBundle] loadNibNamed:@"YuyueDefaultCell" owner:nil options:nil] objectAtIndex:1];
        }
        return cell;
    ...
    TemplateModel* mData = [self.datasArry objectAtIndex:indexPath.row+4];
