...
//    self.leftTitle = @"短彩信推送详情";
    ...
    manager.keyboardDistanceFromTextField = 35;
...
//    [[NSNotificationCenter defaultCenter] removeObserver:self];
    [[NSNotificationCenter defaultCenter] removeObserver:self name:UITextViewTextDidChangeNotification object:self.detailTextF];
    [[NSNotificationCenter defaultCenter] removeObserver:self name:UITextViewTextDidChangeNotification object:self.smsTitleLab];
}
...
- (void)sendRequestToSaveOrModifyWithMsgId:(NSInteger)msgId storeId:(NSString *)storeId {
    ...
    if (self.userNum && ![self.userNum isEqualToString:@""] && self.userNum.length > 0) {
        if ([self.userNum isEqualToString:self.model.targetUser]) {
            userNum = self.model.targetUser;
        }else {
