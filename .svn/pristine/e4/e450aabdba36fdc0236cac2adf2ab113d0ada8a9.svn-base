...
- (void)setUI{
    ...
    [scrollView setContentSize:CGSizeMake(KScreenWidth, 530)];
    self.vipCardView = [[NewVipCardView alloc]initWithType:VipCardNoChoose];
    self.vipCardView.delegate = self;
    self.vipCardView.frame = CGRectMake(0, 12, KScreenWidth, 530);
...
//    }];
    ...
    [scrollView addSubview:self.vipCardView];
...
- (void)didClickTheButton:(NSString *)buttonTitle{
    if ([buttonTitle isEqualToString:@"储值会员卡"]) {
        self.vipCardView.type = VipCardSaving;
    }else if([buttonTitle isEqualToString:@"优惠会员卡"]){
        self.vipCardView.type = VipCardCoupon;
    }else{
        self.vipCardView.type = VipCardNoChoose;
    }
}
//zyc
