...
- (void)setModel:(PartModel *)model{
    ...
    [_imageSender sd_setImageWithURL:[NSURL URLWithString:imagePath] forState:UIControlStateNormal placeholderImage:[UIImage imageNamed:@"addImage"]];
}
...
//    CGFloat titleW = 27;
    [model.titles enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        MenuView* sender = [[MenuView alloc] initWithFrame:CGRectMake(width*j, 8 + (width+20)*i, width, height-2) block:^(id sender) {
