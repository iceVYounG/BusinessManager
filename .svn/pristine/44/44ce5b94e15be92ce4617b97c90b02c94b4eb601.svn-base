...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(keboardShow1) name:UIKeyboardWillShowNotification object:nil];
...
- (void)quick1:(NSNotification *)note{
    ...
    switch (index) {
        case 0:{
            ...
            break;
        case 1:{
        [self ShopInfoDataupload1:self.TemModel];
...
- (void)ShopInfoDataupload1:(FlowerAddressModel *)model{
    PartModel *model1;
    if (self.model) {
        model1=self.model;
    }else{
        ...
        model1.templateNo=self.templeId;
    ...
    [[MallNetManager share] request:API_TemplateSave parameters:[arr jm_ArryPrams] success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        if (Succeed(responseObject)) {
            ...
            if (rsp) {
                if (AccountInfo.isTiyan) {
                    [wSelf updateStoreInforRequest:AccountInfo.tempStoreId targetId:AccountInfo.storeId withTempNo:wSelf.templeId withHongBao:YES];
                }
            }
        }else{
