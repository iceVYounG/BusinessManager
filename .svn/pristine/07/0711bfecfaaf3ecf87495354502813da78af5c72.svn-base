...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.type == TZAssetModelMediaTypeVideo) {
        ...
        [[TZImageManager manager] getOriginalPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info) {
            ...
            CLRatio *ratio = [[CLRatio alloc] initWithValue1:0 value2:0];
            ratio.isLandscape = YES;
            CLImageEditor *editor = [[CLImageEditor alloc] initWithImage:photo delegate:self ratio:ratio];
...
{
//    [[NSNotificationCenter defaultCenter]postNotificationName:@"loadImage" object:image];
    [editor dismissViewControllerAnimated:YES completion:nil];
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;

    if ([tzImagePickerVc.pickerDelegate respondsToSelector:@selector(imagePickerController:didFinishPickingPhotos:sourceAssets:isSelectOriginalPhoto:)]) {
        [tzImagePickerVc.pickerDelegate imagePickerController:tzImagePickerVc didFinishPickingPhotos:@[image] sourceAssets:nil isSelectOriginalPhoto:_isSelectOriginalPhoto];
    }
    if ([tzImagePickerVc.pickerDelegate respondsToSelector:@selector(imagePickerController:didFinishPickingPhotos:sourceAssets:isSelectOriginalPhoto:infos:)]) {
        [tzImagePickerVc.pickerDelegate imagePickerController:tzImagePickerVc didFinishPickingPhotos:@[image] sourceAssets:nil isSelectOriginalPhoto:_isSelectOriginalPhoto infos:nil];
    }
    if (tzImagePickerVc.didFinishPickingPhotosHandle) {
        tzImagePickerVc.didFinishPickingPhotosHandle(@[image],nil,_isSelectOriginalPhoto);
    }
    if (tzImagePickerVc.didFinishPickingPhotosWithInfosHandle) {
        tzImagePickerVc.didFinishPickingPhotosWithInfosHandle(@[image],nil,_isSelectOriginalPhoto,nil);
    }
    [tzImagePickerVc hideProgressHUD];
    [self.navigationController dismissViewControllerAnimated:YES completion:nil];
}
