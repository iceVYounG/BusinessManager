...
extern NSString *const SmartLinkTimeOutNotification;
...
/**
 *  Login an exist account,email or phone number
 *
 ...
 */
- (CloudUserInfo *)loginWithAccount:(NSString *)account withPassword:(NSString *)password error:(NSError **)error;
...
/**
 *  send wifi configuration to camera with smartlink。you shoud addObserver SmartLinkTimeOutNotification。
 *  smartlink timeout is 180S.
 * 
 *  @param ssid             Wi-Fi network name
 *  @param password         Wi-Fi password
 *  @param appKey           Key for QRCode add device, should same as the key in camera
 *  @param wifiSecuritytype type `wifiSecurityType_E`, used to secure the qr code. You can pass WifiSecurityTypeInvalid to invalid secure action.
 *  @param encryptType      type `encryptType_E`, used to encrypt. You can pass EncryptTypeAll to encrypt the wifi configuration send to camera.
 *  @param error            NSError contains error code and description, get description by userInfo objectForKey: NSLocalizedFailureReasonErrorKey.
 *  @return BOOL value, YES means successed start send smartlink message.
 */
- (BOOL)smartlinkWithWiFiSSID:(NSString *)ssid
                 withPassword:(NSString *)password
                 withQRCodeKey:(NSString *)appKey
                  wifiSecurityType:(wifiSecurityType_E)wifiSecuritytype
                     encryptType:(encryptType_E)encryptType
...
/**
 *   destroySmartlink.you must call it after use smartlinkWithWiFiSSID.
 */
- (void)destroySmartlink;
/**
 *  Remove one camera.
 *
...
/**
 *  This function can get closeli web purchase url.you can load this url with your UIWebView.this web purchase support AliPay.
 *  when  webView.request.URL.absoluteString contains  "appPaySuccess",deal done.
 *
 *  @param cameraDevice the camera to get web purchase url
 *
 *  @return NSURL，which you can load with UIWebView.
 */
- (NSURL *)getCloseliWebPurchaseUrl:(CloseliCameraDevice *)cameraDevice;
/**
