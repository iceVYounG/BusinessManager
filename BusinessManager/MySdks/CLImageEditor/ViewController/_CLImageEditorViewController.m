...
- (id)initWithImage:(UIImage*)image delegate:(id<CLImageEditorDelegate>)delegate ratio:(CLRatio *)ratio
{
    self = [self init];
    if (self){
        _originalImage = [image deepCopy];
...
- (void)initNavigationBar
{
    ...
    [_navigationBar setBarTintColor:[UIColor colorWithHexString:@"#3a3a3a"]];
    ...
    if(IOS7){ _navigationBar.translucent = NO; }
...
- (void)viewDidLoad
{
    ...
    [self setupToolWithToolInfo:_menuView.subviews[0].toolInfo];
...
- (void)setCurrentTool:(CLImageToolBase *)currentTool
{
    if(currentTool != _currentTool){
        ...
        _currentTool.clipRatio = _clipRatio;
