...
@property (nonatomic, strong) UIView *titeBgView;
...
+ (void)initialize
{
    ...
    appearance.titleFont = [UIFont boldSystemFontOfSize:14];
...
- (id)initWithTitle:(NSString *)title andMessage:(NSString *)message textAliment:(NSTextAlignment)textAlignment
{
    self = [super init];
    if (self) {
        _title = title;
...
+ (void)showBackground
{
    if (!__si_alert_background_window) {
        ...
        //针对iOS9需要设置rootViewController
        UIViewController *vc = [[UIViewController alloc]initWithNibName:nil bundle:nil];
        __si_alert_background_window.rootViewController = vc;
        //===============================
...
- (void)validateLayout
{
	...
	if (self.titleLabel) {
        ...
        if (!self.titeBgView) {
            ...
            [view setBackgroundColor:[UIColor colorWithHexString:@"F4F5F0"]];
            [self.containerView insertSubview:view belowSubview:self.titleLabel];
            
            self.titeBgView = view;
        }
        self.titeBgView.frame = CGRectMake(0, 0, CONTAINER_WIDTH, self.titleLabel.frame.origin.y * 2 + self.titleLabel.frame.size.height);
        UIBezierPath *maskPath = [UIBezierPath bezierPathWithRoundedRect:self.titeBgView.bounds byRoundingCorners:UIRectCornerTopLeft | UIRectCornerTopRight cornerRadii:CGSizeMake(10, 10)];
        CAShapeLayer *maskLayer = [[CAShapeLayer alloc] init];
        maskLayer.frame = self.titeBgView.bounds;
        maskLayer.path = maskPath.CGPath;
        self.titeBgView.layer.mask = maskLayer;
        /////////////////
...
//            paragraphStyle.lineBreakMode = self.messageLabel.lineBreakMode;
            ...
            return MAX(minHeight, ceil(rect.size.height)+10);
        #else
