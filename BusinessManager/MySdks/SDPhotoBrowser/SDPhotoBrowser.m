...
@implementation SDPhotoBrowser 
{
    ...
    BOOL _willDisappear;
    BOOL _changeSaveBtnColor;// 改变保存按钮的颜色(置灰)
    NSInteger _nowIndex;// 当前的图片index
}
...
- (void)setupToolbars
{
    ...
    for (int i = 0; i<self.imageCount; i++) {
        [self.colorArr addObject:@"0"];
    }
//    NSLog(@"_______self.colorArr = %@",self.colorArr);
...
//    saveButton.backgroundColor = [UIColor colorWithRed:0.1f green:0.1f blue:0.1f alpha:0.90f];
    [saveButton setBackgroundImage:[UIImage imageNamed:@"save_bg"] forState:UIControlStateNormal];
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo;
{
    ...
    if (error) {
        ...
        [self.colorArr replaceObjectAtIndex:_nowIndex withObject:@"1"];
        [_saveButton setTitleColor:[UIColor grayColor] forState:UIControlStateNormal];
        
        _changeSaveBtnColor = YES;
    }
...
- (void)setupImageOfImageViewForIndex:(NSInteger)index
{
    ...
    if ([self highQualityImageURLForIndex:index]) {
        ...
        if (self.type == 1) {
            ...
            imageView.image = [self.delegate photoBrowser:self imageURLForIndex:index];
        }
...
- (void)layoutSubviews
{
    ...
    _saveButton.frame = CGRectMake(30, self.bounds.size.height - 60, 60, 45);

//    _saveButton.imageEdgeInsets = UIEdgeInsetsMake(0, 0, 0, 0);
//    _saveButton.titleEdgeInsets = UIEdgeInsetsMake(0, 31, 0, 1);
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (!_willDisappear) {
        _nowIndex = index;// 当前序号
        if ([self.colorArr[index] isEqualToString:@"1"]) {
            _changeSaveBtnColor = YES;
            [_saveButton setTitleColor:[UIColor grayColor] forState:UIControlStateNormal];
