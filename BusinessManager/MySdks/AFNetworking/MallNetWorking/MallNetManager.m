...
static MallNetManager *_sharedManager = nil;
...
@implementation MallNetManager
+ (instancetype)share {
    
    dispatch_once(&onceToken, ^{
        //设置服务器根地址
        _sharedManager = [[MallNetManager alloc] initWithBaseURL:[NSURL URLWithString:[URL_Server stringByAppendingString:URL_Root]]];
...
-(void)addCustomHeader:(MallNetManager*)manager{
...
- (NSURLSessionDataTask *)request:(NSString *)URLString
    ...
    MallNetManager *manager = [MallNetManager share];
...
-(NSURLSessionDataTask *)NewFloorRequest:(NSString*)urlStr params:(id)params succeed:(void (^)(id responseObject))success showHUD:(BOOL)showHUD {
    ...
    if (showHUD) {
        [SVProgressHUD showWithStatus:@"图片上传成功，正在保存商品信息..."];
        [SVProgressHUD setDefaultMaskType:SVProgressHUDMaskTypeGradient];
        [SVProgressHUD setDefaultStyle:SVProgressHUDStyleDark];
        
    }
    return [self request:urlStr parameters:params success:^(NSURLSessionDataTask *task, id responseObject) {
        [SVProgressHUD dismiss];
        if (Succeed(responseObject)) {
            if (success) {
                success(responseObject);
