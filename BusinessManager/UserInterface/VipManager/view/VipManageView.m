...
//    }];
    ...
    //
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshMemberListViewData:) name:@"VIPCARD_REFRESHMEMBERLIST" object:nil];
...
#pragma mark - notification Method - refresh
- (void)refreshMemberListViewData:(NSNotification *)notifi {
    NSString *name = notifi.name;
    NSDictionary *userInfo = (NSDictionary *)notifi.userInfo;
    CardType cardType = [[userInfo objectForKey:@"cardType"] integerValue];
    if ([name isEqualToString:@"VIPCARD_REFRESHMEMBERLIST"]) {
        if (cardType == CardType_Chuzhika) {
            [self.savingCardTableView.mj_header beginRefreshing];
            NSLog(@"-----储值卡 刷新啦------");
        }else {
            [self.couponCardTableView.mj_header beginRefreshing];
            NSLog(@"-----优惠卡 刷新啦------");
        }
    }
}
...
- (void)setModel:(VipCardVipListModel *)model {
    ...
    if (model.DISCOUNT && ![model.DISCOUNT isEqual:[NSNull class]]) {
        double discount = [model.DISCOUNT doubleValue];
        if (discount < 10.0) {
