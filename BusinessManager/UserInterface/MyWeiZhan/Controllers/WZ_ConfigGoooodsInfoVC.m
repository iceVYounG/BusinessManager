   ...
   __block NSMutableArray *imageArr=[NSMutableArray array];
    ...
    if (isAddNewImage) {
        [self showCameraOrMutilPhotoPicker:^(NSMutableArray *imageArray) {
            [imageArr addObjectsFromArray:imageArray];
            if (isAddNewImage && indexpath.item == 0) {
                if (imageArray.count > 0) {
                    ...
                    [imageArr replaceObjectAtIndex:0 withObject:cutImg];
                }else {
                    return;
                }
            }
            ...
            [weakSelf uploadMultiImageMethod:imageArr selectedIndexPath:indexpath andIsAddNewImage:isAddNewImage];
        }];
