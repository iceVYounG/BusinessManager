...
@interface YuyueBaoYangVC ()<UITableViewDelegate,UITableViewDataSource,ShopEditCellDeledate>
@property(nonatomic,strong)UITableView* tableView;
@end

@implementation YuyueBaoYangVC
...
-(void)setModel:(PartModel *)model{
    ...
    if (self.datasArry.count==0) {
        NSMutableArray *codeArr=[NSMutableArray arrayWithObjects:@"yyby1",@"yyby2",@"yyby3",@"yyby4",@"yyby5", nil];
        NSMutableArray *valueArr=[NSMutableArray arrayWithObjects:@"联系人姓名",@"联系人手机号码",@"需要保养的车型",@"保养的时间",@"需要保养的项目",nil];
        NSMutableArray *typeArr=[NSMutableArray arrayWithObjects:@"1",@"1",@"1",@"2",@"1", nil];
        for (int i=0; i<5; i++) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section==0) {
        NSString *cellId = @"baoYangCell";
        YuyueBaoYangCell *cell = [tableView dequeueReusableCellWithIdentifier:cellId];
        if (!cell) {
            cell = [[[NSBundle mainBundle] loadNibNamed:@"YuyueDefaultCell" owner:nil options:nil] objectAtIndex:2];
        }
        return cell;
        
    }
    else{
    TemplateModel* mData = [self.datasArry objectAtIndex:indexPath.row+5];
...
-(void)requestUpload{
    ...
    if (!self.model) {
        ...
        model.templateModelname = @"预约保养";
        model.templateModelnameCode = @"yyby";
