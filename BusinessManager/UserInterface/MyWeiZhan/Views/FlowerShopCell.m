...
- (void)setModel:(PartModel *)model{
    ...
    if (model) {
        ...
        [self.imageV sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",ImagePre,imgStr]] placeholderImage:[UIImage imageNamed:@"default_img"]];
        NSLog(@">>%@",imgStr);
...
- (void)refreshUI{
    ...
    if (!self.temModel.address) {
        self.temModel.address=[[TemplateModel alloc]init];
    }
    ...
    self.nameTF.placeholder=@"请输入您的店铺名称";
    ...
    self.phoneTF.placeholder=@"请输入您的联系电话";
...
- (instancetype)initWithFrame:(CGRect)frame{
    if (self=[super initWithFrame:frame]) {
        ...
        self.myScro.localizationImagesGroup=@[[UIImage imageNamed:@"default_img"]];
...
- (void)setModel:(TemplateModel *)model{
    _model=model;
    if (model) {
        [self.imgV sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",ImagePre,model.imgPath]] placeholderImage:[UIImage imageNamed:@"default_img"]];
