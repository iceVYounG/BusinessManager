...
- (void)viewDidLoad {
    ...
    if (AccountInfo.dianShangName.length>0) {
        self.userphonetext.text =  NoNullStr(AccountInfo.dianShangName, @"");
    }
    else{
        self.userphonetext.text =  NoNullStr(AccountInfo.linkPhone, @"");
    }
    NSLog(@"%@",AccountInfo.dianShangName);
...
//    NewLoginVC *vc=[[NewLoginVC alloc]initWithNibName:@"NewLoginVC" bundle:nil];
...
//    [self.smscodebtn setTitle:@"获取短信验证码" forState:UIControlStateNormal];
...
-(void)sendSmsCode:(UIButton *)sender{
    ...
    if ([self.userphonetext.text length] == 11 && [self.userphonetext.text judgeTelephone]) {
        ...
        NSLog(@">>>>>>>>yes  or no  %d<<<<<<<",[self.userphonetext.text judgeTelephone]);
        downtimer = 60;
...
//    [[MallNetworkEngine loginshare] requestNotEncodeWithDate:self.requestDate requestWithPath:API_LoginHost Params:manger CompletionHandler:^(MKNetworkOperation *completedOperation) {
    ...
    [[MallNetworkEngine loginshare] requestNotEncodeWithDate:self.requestDate requestWithPath:API_LoginHost Params:manger CompletionHandler:^(MKNetworkOperation *completedOperation) {
        ...
        if ([[dic objectForKey:@"code"] isEqualToString:@"00-00"])  {
            ...
            if ([[tempdic objectForKey:@"b2cStoreId"] isKindOfClass:[NSNull class]]) {
